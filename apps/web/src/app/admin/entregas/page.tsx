...
function ZonesTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="bg-white border border-primary/20 p-5 space-y-4">
          <h3 className="font-semibold text-primary text-sm">{editingId ? 'Editar zona' : 'Nueva zona'}</h3>
        ...
        <div className="bg-white border border-primary/10">
          <table className="w-full text-sm">
            <thead className="bg-muted/50 border-b border-primary/10">
              <tr>
                <th className="text-left px-4 py-3 text-xs uppercase tracking-widest text-primary/50">Zona</th>
                <th className="text-left px-4 py-3 text-xs uppercase tracking-widest text-primary/50">Tarifa</th>
                <th className="text-left px-4 py-3 text-xs uppercase tracking-widest text-primary/50">Barrios</th>
            ...
            <tbody className="divide-y divide-primary/5">
              {zones.map((z) => (
                <tr key={z.id} className="hover:bg-surface/50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-right space-x-3">
                    <button onClick={() => openEdit(z)} className="text-xs text-accent hover:underline">Editar</button>
                    {confirmDelete === z.id ? (
                      <>
                      ...
                      <button onClick={() => setConfirmDelete(z.id)} className="text-xs text-primary/30 hover:text-red-500">Eliminar</button>
                    )}
                  </td>
                </tr>
              ))}
              {zones.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-10 text-center text-sm text-primary/40">No hay zonas.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function SlotsTab() {
  ...
  return (
    <div className="space-y-4">
      ...
      {showForm && (
        <div className="bg-white border border-primary/20 p-5 space-y-4">
          <h3 className="font-semibold text-primary text-sm">{editingId ? 'Editar franja' : 'Nueva franja'}</h3>
          {formError && <p className="text-red-500 text-sm">{formError}</p>}
          <div className="grid grid-cols-2 gap-4">
            <div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white border border-primary/10">
          <table className="w-full text-sm">
            <thead className="bg-muted/50 border-b border-primary/10">
              <tr>
                <th className="text-left px-4 py-3 text-xs uppercase tracking-widest text-primary/50">Franja</th>
                <th className="text-left px-4 py-3 text-xs uppercase tracking-widest text-primary/50">Horario</th>
                <th className="text-center px-4 py-3 text-xs uppercase tracking-widest text-primary/50">Pos.</th>
                <th className="text-center px-4 py-3 text-xs uppercase tracking-widest text-primary/50">Estado</th>
                <th className="px-4 py-3" />
              </tr>
            </thead>
            <tbody className="divide-y divide-primary/5">
              {slots.map((s) => (
                <tr key={s.id} className="hover:bg-surface/50 transition-colors">
                  <td className="px-4 py-3 font-medium text-primary">{s.label}</td>
                  <td className="px-4 py-3 text-primary/60">{s.startTime} — {s.endTime}</td>
                  <td className="px-4 py-3 text-center text-primary/60">{s.position}</td>
                  <td className="px-4 py-3 text-center">
                  ...
                  <td className="px-4 py-3 text-right space-x-3">
                    <button onClick={() => openEdit(s)} className="text-xs text-accent hover:underline">Editar</button>
                    <button onClick={() => deleteMutation.mutate(s.id)} className="text-xs text-primary/30 hover:text-red-500">Eliminar</button>
                  </td>
                </tr>
              ))}
              {slots.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-10 text-center text-sm text-primary/40">No hay franjas horarias.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function BlockedDatesTab() {
  ...
  return (
    <div className="space-y-4">
      {/* Quick-add form */}
      <div className="bg-white border border-primary/20 p-4 flex gap-4 items-end">
        <div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white border border-primary/10">
          <table className="w-full text-sm">
            <thead className="bg-muted/50 border-b border-primary/10">
              <tr>
                <th className="text-left px-4 py-3 text-xs uppercase tracking-widest text-primary/50">Fecha</th>
                <th className="text-left px-4 py-3 text-xs uppercase tracking-widest text-primary/50">Motivo</th>
                <th className="px-4 py-3" />
              </tr>
            </thead>
            <tbody className="divide-y divide-primary/5">
              {blocked.map((b) => (
                <tr key={b.id} className="hover:bg-surface/50">
                  ...
                  <td className="px-4 py-3 text-primary/60">{b.reason ?? '—'}</td>
                  <td className="px-4 py-3 text-right">
                    <button onClick={() => deleteMutation.mutate(b.id)} className="text-xs text-primary/30 hover:text-red-500">Desbloquear</button>
                  </td>
                </tr>
              ))}
              {blocked.length === 0 && (
                <tr><td colSpan={3} className="px-4 py-10 text-center text-sm text-primary/40">Sin fechas bloqueadas.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
export default function EntregasPage() {
  ...
  return (
    <div className="p-8 max-w-5xl">
      <h1 className="font-display text-2xl text-primary mb-6">Configuración de entregas</h1>
